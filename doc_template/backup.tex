...
\begin{abstract}
When humans view a scene containing multiple objects, emotionally arousing
stimuli tend to capture attention more readily than neutral ones---a phenomenon
known as arousal-biased competition. Vision-language models trained on
large-scale human-generated image-text pairs may have acquired similar biases
through selective image description in training data. We tested this by
presenting six CLIP models with composite stimuli containing pairs of
affectively rated images, and quantifying representational priority using two
complementary metrics: within-constituent attention rollout and
composite-constituent embedding similarity. Image-level linear regressions
showed that human arousal ratings consistently predicted representational priority
across all models, both metrics, and three independent affective image datasets,
even after controlling for low-level visual features. Valence effects were
weaker and less consistent. These results suggest that large-scale multimodal
training may transmit not only semantic correspondences but also human-like
affective biases in visual selective attention.
\end{abstract}

%----------------------------------------------------------------------
\section{Introduction}
%----------------------------------------------------------------------
Many practical applications of vision-language models expect comprehensive, objective representations of visual scenes. However, research has shown that vision-language models such as CLIP do not encode visual content equally, but instead exhibit systematic differences in representational priority. For example, \citet{abbasiCLIPMicroscopeFineGrained2025} found that in multi-object retrieval tasks, CLIP's image encoder is biased toward larger objects, while its text encoder is biased toward objects mentioned earlier. These findings suggest that CLIP's representations are not evenly balanced, but are instead shaped by presentational factors such as object size and order of mention.

Yet presentational factors alone cannot fully address a deeper question: does CLIP also have intrinsic preferences driven by content itself? That is, when different content is presented in the same way and controlled for low-level visual features, does the model still prioritize certain types of content? Specifically, we ask whether the psychological properties of visual content shape model representational priority.
...
We propose that a similar affective representational bias may be inherited by vision-language models through their training process. Models such as CLIP are trained on large-scale human-generated image--text pairs \citep{radfordLearningTransferableVisual2021a}. Image captions are usually not exhaustive records of visual scenes, but selective ones: people tend to describe what they find most salient and noteworthy. If high-arousal content is more likely to capture human attention and thus more likely to be mentioned in captions, contrastive image--text training may push models to assign such content higher representational priority. %In this way, models may learn not only the semantic correspondences between images and text, but also the attentional biases embedded in human description behavior.
...
To examine this, we constructed composite stimuli by placing pairs of affectively rated images side by side and presented them to six CLIP models with ViT-based visual encoders. We quantified how each model allocated representational priority between the two constituent images using two complementary metrics: composite-to-constituent embedding similarity and within-constituent attention rollout. Image-level regression analyses showed that arousal consistently predicted model representational priority across models, metrics, and datasets, while the effect of valence was weaker and less consistent. These findings suggest that vision-language models may not only learn visual-semantic knowledge, but also inherit human-like affective attentional biases.
...
(LAION-2B~\citep{schuhmann2022laion5bopenlargescaledataset}
and DataComp-1B~\citep{gadre2023datacompsearchgenerationmultimodal}), 
giving a $3 \times 2$ design. 
We analyzed only the CLIP visual encoder and excluded the text pathway, since our question concerns visual representational priority.
...
($r_\text{valence} = .984$, $r_\text{arousal} = .929$). 
...
First, its images span a broad affective space with near-zero correlation between valence and arousal ($r = -.06$; \cref{fig:distribution}), minimizing collinearity problem in the
regression analysis. 
...
In contrast, images in the supplementary datasets have varying non-square ratios. 
Third, OASIS places no restrictions on image display and provides a concept label for each image, allowing us to present both image-level instances and concept-level summaries directly.
...
For each dataset, we randomly sampled 100,000 image-pair permutations, allowing each image to appear across diverse pairing contexts and positions.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{plots/Fig1_schematic.pdf}
  \caption{Two complementary metrics of representational priority in CLIP visual
    encoders. 
...
We used two complementary metrics to quantify model representational priority
(\cref{fig:schematic}).

\parhead{Within-constituent attention rollout.}
Attention rollout~\citep{attentionrollout_abnar2020} estimates cumulative
information flow among tokens. It recursively multiplies attention matrices across transformer layers, including residual connections. For layer $l$ with attention matrix $A^{(l)}$
\begin{align}
...
To obtain a constituent-level measure, we use the \texttt{[CLS]} row of the rollout matrix and average its values over all patch tokens belonging to constituent $i$. 
...
Within each trial, constituent scores were normalized to sum to 1, giving
relative priorities $\tilde{s}_i = s_i / (s_i + s_j)$. Each image's overall
representational priority was defined as the mean $\tilde{s}_i$ across all trials in which it appeared.
...
predicted representational priority. We control low-level visual features:
...
Before examining regression coefficients, we first inspected which items received high or low representational priority.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{doc/plots/Fig2_instances.pdf}
  \caption{OASIS images and concept labels associated with representational priority.
...
We next test whether affective ratings predict image-level representational priority after controlling for low-level visual features. \Cref{fig:oasis_coefs} shows standardized regression coefficients for OASIS across six CLIP models and two representational priority metrics.

Arousal shows a consistent positive effect. 
Across all models and both metrics, higher arousal predicts higher representational priority, even after controlling for visual features.
...
\begin{figure}[h]
  ...
  \caption{Standardized coefficients predicting image-level representational priority for OASIS. 
  Each point shows the coefficient from one CLIP model. 
  Row panels correspond to the two representational priority metrics. 
...
\Cref{fig:landscape} visualizes model representational priority across the
valence--arousal space for all three datasets and both metrics. 
Across datasets and metrics, high-priority regions tend to occur at higher arousal values, consistent with the regression results from OASIS. Valence also shows a positive trend, but the pattern is less pronounced than arousal.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{plots/Fig4_affect_landscape.pdf}
  \caption{Affective landscape of model representational priority across datasets and metrics. 
...
In most model--metric combinations, both arousal and valence positively predict representational priority, with the main exception being embedding similarity in OASIS.
...
\begin{figure}[h]
  ...
  Filled circles: Bonferroni-corrected $p < .05$; open circles: n.s.}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{S1_distribution.png}
  \caption{Valence--arousal distribution for each dataset. Each point
    ...
    arousal is shown in each panel. OASIS shows a near-zero correlation
    ($r = -.06$), whereas NAPS ($r = -.82$) and EmoMadrid ($r = -.58$)
    show stronger negative associations. THINGS is included for reference
    but was not used in the present analyses.}
  \label{fig:distribution}
\end{figure}

\section{Per-dataset Regression Coefficients}
...
\begin{figure}[h]
  ...
  \caption{Full regression coefficients for EmoMadrid, including all visual
    feature controls. Filled circles: Bonferroni-corrected $p < .05$;
    open circles: n.s.}
